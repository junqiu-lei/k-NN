...
public class SpaceTypeTests extends KNNTestCase {
    ...
    public void testGetSpace_whenUndefined_thenThrowException() {
        Exception ex = expectThrows(IllegalArgumentException.class, () -> SpaceType.getSpace(SpaceType.UNDEFINED.name()));
        assertTrue(ex.getMessage().contains("Unable to find"));
    }

    public void testIsSupported_whenCalled_thenReturn() {
        Map<SpaceType, Set<VectorDataType>> expected = Map.of(
            ...
            Set.of(VectorDataType.FLOAT, VectorDataType.BYTE, VectorDataType.BINARY)
        );

        for (SpaceType spaceType : SpaceType.values()) {
            for (VectorDataType vectorDataType : VectorDataType.values()) {
                if (expected.get(spaceType).isEmpty()) {
                    Exception ex = expectThrows(IllegalStateException.class, () -> spaceType.isSupported(vectorDataType));
                    assertTrue(ex.getMessage().contains("should not be called"));
                } else {
                    assertEquals(expected.get(spaceType).contains(vectorDataType), spaceType.isSupported(vectorDataType));
                }
            }
        }
    }
